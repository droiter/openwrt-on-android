...
mac80211_prepare_vif() {
	...
	if [ x"$ifname" = x ] ; then
           #by yacc
           #for xwdev in $(list_phy_wlan "$phy"); do
           #     ifname=$xwdev
           #     break
           #done
           ifname="$(get_phy_interface $phy)"
        fi
	...
	case "$mode" in
		...
		mesh)
			...
			if [ -n "$key" ]; then
				...
				#iw phy "$phy" interface add "$ifname" type mp mesh_id "$mesh_id"
                                iw dev "$ifname" set type mesh
                                iw dev "$ifname" set meshid "$mesh_id"
			fi
		;;
		monitor)
		...
		sta)
            ...
            iw dev "$ifname" set type managed $wdsflag
...
list_phy_wlan() {
	local phy="$1"
	if [ -d "/sys/class/ieee80211/${phy}/device/net" ]; then
		ls "/sys/class/ieee80211/${phy}/device/net" | grep wlan 2>/dev/null;
	else
		ls "/sys/class/ieee80211/${phy}/device" 2>/dev/null | grep net: | sed -e 's,net:,,g'
	fi
}

# same as wifi/mac80211.sh
get_phy_interface() {
        local phy="$1"
        if [ -d "/sys/class/ieee80211/${phy}/device/net" ]; then
